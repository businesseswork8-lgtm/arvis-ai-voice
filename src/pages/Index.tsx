...
export default function Index() {
  ...
  const confirmAll = async () => {
    const saved: SavedItem[] = items.map((i) => ({
      ...i, confirmed: true, savedAt: new Date().toISOString(),
    }));
    await saveItems(saved);
    setItems([]);
          ...
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
            ...
            <div className="max-w-md mx-auto px-4 pt-6 pb-24 space-y-3">
              <div className="flex items-center justify-between mb-2">
                <p className="text-sm text-muted-foreground">{activeItems.length} item{activeItems.length !== 1 ? "s" : ""} extracted</p>
                <button onClick={confirmAll}
                  ...
                  <CheckCheck className="w-4 h-4" /> Confirm All
                </button>
              </div>
              <AnimatePresence>
