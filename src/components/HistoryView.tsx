      ...
      {loading ? (
        ...
        <>
          <div className="flex gap-2 p-4 overflow-x-auto no-scrollbar">
            ...
            {folders.map((f) => (
              <Chip key={f.key} label={`${f.emoji} ${f.label}`} active={activeFolder === f.key} onClick={() => setActiveFolder(f.key)} />
            ))}
          </div>
          <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-3">
            <AnimatePresence>
              {filtered.length === 0 && <p className="text-center text-muted-foreground mt-12">No items yet</p>}
              {filtered.map((item, i) => (
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.03 }}
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-muted-foreground">{folder?.emoji} {folder?.label}</span>
      </div>
      <p className="text-sm font-medium text-foreground">{item.title}</p>
      <p className="text-xs text-muted-foreground line-clamp-2">{item.content}</p>
      {item.datetime && <p className="text-xs text-primary">{new Date(item.datetime).toLocaleString()}</p>}
      <p className="text-xs text-muted-foreground/60">Saved {new Date(item.savedAt).toLocaleDateString()}</p>
    </motion.div>
  );
}
